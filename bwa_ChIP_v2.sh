...
#Usage ./bwa_ChIP_v2.sh Threads target_dir 2>&1 | tee mapping_report.log
...
java -jar $HOMEDIR/bin/Trimmomatic-0.35/trimmomatic-0.35.jar SE -threads $THR -phred33 $NAME.fq.gz $NAME"_clipped".fastq ILLUMINACLIP:$HOMEDIR/bin/Trimmomatic-0.35/adapters/ChIP_Seq_Bioo.fa:2:30:10 MINLEN:20

echo -e "${gr}Mapping with bwa...${nc}"
bwa aln -t $THR -n 3 -k 2 -R 300 ~/Genomes/hg38/hg38.fa $NAME"_clipped".fastq > $NAME.sai

done

echo -e "${red}Parallel generation of aligments...${nc}"
parallel -j 5 bwa samse -n 3 ~/Genomes/hg38/hg38.fa {} {.}"_clipped".fastq -f {.}.sam ::: *.sai
...
ls *.unsorted.bam | parallel -j 10 bwa_ChIP_v2_engine.sh {}
